...
const ChatPreview = ({ name, lastMessage, sentiment, time }) => {
  const getSentimentStyles = (s) => {
    switch (s) {
      case 'positive': return { iconColor: '#3B82F6', bgColor: 'rgba(59, 130, 246, 0.1)', text: '95% Confident' };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <ArrowRight className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: '#3B82F6' }} />
      </div>
    </motion.div>
  );
};
